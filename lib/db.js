...
export function getDb(forceReload = false) {
  try {
    if (fs.existsSync(dataFilePath)) {
      ...
      if (!Array.isArray(memoryDb.promoPresets)) memoryDb.promoPresets = [];
      if (!Array.isArray(memoryDb.targetGroups)) memoryDb.targetGroups = [];
      if (!Array.isArray(memoryDb.blacklistedProducts)) memoryDb.blacklistedProducts = [];
      if (!Array.isArray(memoryDb.blacklistedVariants)) memoryDb.blacklistedVariants = [];
...
/**
 * Cek apakah produk atau varian tertentu masuk daftar blacklist (dihapus oleh admin)
 * agar tidak dimunculkan kembali saat sedot supplier / sync data
 */
export function isItemBlacklisted(productName, variantName, db) {
  if (!db) return false;
  const pName = (productName || '').trim().toLowerCase();

  // 1. Cek apakah seluruh produk diblacklist
  const blacklistedProducts = db.blacklistedProducts || [];
  for (const bp of blacklistedProducts) {
    const bpName = typeof bp === 'string' ? bp.trim().toLowerCase() : (bp.name || '').trim().toLowerCase();
    if (bpName && (bpName === pName || (pName.length >= 3 && bpName.includes(pName)) || (bpName.length >= 3 && pName.includes(bpName)))) {
      return true;
    }
  }

  // 2. Cek apakah varian spesifik diblacklist
  if (variantName) {
    const blacklistedVariants = db.blacklistedVariants || [];
    const vName = standardizeDuration((variantName || '').trim().toLowerCase());

    for (const bv of blacklistedVariants) {
      const bvP = (bv.productName || '').trim().toLowerCase();
      const bvV = standardizeDuration((bv.variantName || '').trim().toLowerCase());

      const pMatch = !bvP || bvP === pName || bvP.includes(pName) || pName.includes(bvP);
      if (pMatch && bvV) {
        if (bvV === vName) return true;
        const cleanBvV = bvV.replace(/[^a-z0-9]/g, '');
        const cleanVName = vName.replace(/[^a-z0-9]/g, '');
        if (cleanBvV && cleanBvV === cleanVName) return true;
      }
    }
  }
...
export function standardizeDuration(name) {
  ...
  // --- Normalize equivalent durations (e.g. 30d = 1m) ---
  // 30d = 1 month
  s = s.replace(/\b30d\b/gi, '1m');
  // 60d = 2 months
  s = s.replace(/\b60d\b/gi, '2m');
  // 90d = 3 months
  s = s.replace(/\b90d\b/gi, '3m');
  // 365d = 1 year
  s = s.replace(/\b365d\b/gi, '1y');
  // 12m = 1 year
  s = s.replace(/\b12m\b/gi, '1y');
  // 24m = 2 years
  s = s.replace(/\b24m\b/gi, '2y');
  // 36m = 3 years
  s = s.replace(/\b36m\b/gi, '3y');
...
export function saveDb(data) {
  ...
  try {
    ...
    // Regenerasi public-catalog.json (snapshot statis untuk fallback Vercel)
    // File ini di-commit ke git sehingga selalu tersedia di production
    try {
      ...
      for (const p of (data.products || [])) {
        const vars = (data.variants || []).filter(v => v.productId === p.id);
      ...
      const snapshotPath = path.join(path.dirname(dataFilePath), 'public-catalog.json');
      ...
      // Snapshot tidak kritis, abaikan error
      console.warn('[DB] Snapshot catalog write failed:', snapErr.message);
    }
  } catch (err) {
...
export async function pushToGoogleSheet() {
  ...
  for (const prod of db.products || []) {
    ...
    if (vars.length === 0) {
      // Lewati produk yang tidak memiliki varian agar tidak muncul produk dummy Rp 0 di katalog
      continue;
    }
...
export function mergeSheetIntoDb(sheetItems) {
  ...
  for (const row of sheetItems) {
    ...
    // Cek blacklist: Jangan masukkan kembali produk atau varian yang sudah dihapus oleh admin
    if (isItemBlacklisted(prodName, varName, db)) {
      continue;
    }
