...
import { getDb, saveDb, pushToGoogleSheet, standardizeDuration, isItemBlacklisted } from './db.js';
...
function processScrapedItem({ productName, variantName, costPrice, stock, isAvailable, description, supplierName, context, index, idPrefix = 'item' }) {
  ...
  // Cek blacklist: jika produk atau varian ini pernah dihapus admin, abaikan dan jangan masukkan lagi
  if (isItemBlacklisted(productName, variantName, db)) {
    return;
  }
...
export async function scrapeTelegramSupplier(options = {}) {
  ...
  try {
    ...
    if (scrapedProducts.length > 0) {
      // Pertahankan produk kustom yang dibuat manual oleh admin (selama tidak diblacklist)
      const customProducts = (db.products || []).filter(p => !scrapedProducts.some(sp => sp.id === p.id) && !isItemBlacklisted(p.name, null, db));
      const customVariants = (db.variants || []).filter(v => {
        if (scrapedVariants.some(sv => sv.id === v.id)) return false;
        const parentP = (db.products || []).find(p => p.id === v.productId);
        const pName = parentP ? parentP.name : '';
        return !isItemBlacklisted(pName, v.name, db);
      });
...
export async function importParsedProducts(items) {
  ...
  for (const item of items) {
    if (isItemBlacklisted(item.productName, item.variantName, db)) {
      continue;
    }
